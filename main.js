...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.lastSaveData='';
	//
// PASTE THIS CODE OVER THE EXISTING Game.WriteSave FUNCTION
//
...
Game.WriteSave=function(type)
{
    ...
    // --- MODIFIED SAVE LOGIC WITH DEBUGGING ---
	var saveDataString = escape(utf8_to_b64(str) + '!END!');
	...
	else
	{
            ...
            if (user && db) {
                ...
                .then(() => {
                    if (document.hasFocus()) {
                        Game.Notify('Game saved to cloud', '', '', 1, 1);
                    }
                })
                .catch((error) => {
                    console.error("Firebase Error: ", error.code, error.message); // DEBUG LINE
                    Game.Notify('Error saving to cloud', 'Please export your save manually!', '', 6, 1);
                });
            } else {
                console.log("User not logged in. Saving locally."); // DEBUG LINE
                localStorageSet(Game.SaveTo, saveDataString);
                if (document.hasFocus()) {
                    if (user) Game.Notify('Game saved locally', 'Could not connect to cloud.', '', 1, 1);
                    else Game.Notify('Game saved locally', 'Log in to save to the cloud.', '', 1, 1);
                }
            }
        } catch(e) {
            console.error("Firebase not available. Saving locally.", e);
            localStorageSet(Game.SaveTo, saveDataString);
        }
	}
}
		...
		*/
		...
		// ======================================================
// === STEP 3: REPLACE THE INITIAL LOAD LOGIC AT THE END OF Game.Init ===
// ======================================================
// --- MODIFIED INITIAL LOAD LOGIC ---
auth.onAuthStateChanged(user => {
    if (user && db) {
        // User is signed in, load from Firestore
        console.log("User logged in. Loading from cloud...");
        db.collection('saves').doc(user.uid).get().then(doc => {
            if (doc.exists && doc.data().saveData) {
                if (Game.ImportSaveCode(doc.data().saveData)) {
                    Game.Notify('Game loaded from cloud', '', '', 1, 1);
                } else {
                    // Save was invalid, load local as fallback
                    Game.LoadSave();
                }
            } else {
                console.log("No cloud save found. Loading local save if available.");
                Game.LoadSave(); // Fallback to local
            }
        }).catch(error => {
            console.error("Error loading from cloud: ", error);
            Game.LoadSave(); // Fallback to local on error
        });
    } else {
        // User is signed out OR Firebase isn't ready, load from localStorage
        console.log("User not logged in or Firebase not ready. Loading local save.");
        Game.LoadSave();
    }
});
